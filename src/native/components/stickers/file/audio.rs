...
const MATRIX_IDLE_ALPHA: f32 = 0.4;
...
impl AudioHandle {
    fn take_event_rx(&mut self) -> Option<mpsc::Receiver<AudioEvent>> {
        self.event_rx.take()
    }
}
...
pub(super) struct AudioState {
    ...
    current_idx: usize,
    siblings_loaded: bool,
    anim_loop_started: bool,
    frame_metrics: AudioFrameMetrics,
...
impl AudioState {
    ...
    fn load(&mut self, path: PathBuf) {
        if let Some(handle) = &self.handle {
            ...
            let mut handle = build_handle(path);
...
impl super::FileSticker {
    pub(super) fn load_audio(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        if let Some(state) = self.state_mut() {
            state.load(path.clone());
            self.spawn_load_metadata(path, cx);
        }
    }
    ...
    fn switch_play_mode(&mut self, cx: &mut Context<Self>) {
        if let Some(state) = self.state_mut() {
    ...
    fn navigate_with_mode(&mut self, delta: i64, allow_random: bool, cx: &mut Context<Self>) {
        ...
        self.summaries = vec![super::summary::FileSummary::new(&new_path_str)];
    ...
    fn spawn_load_metadata(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        if let Some(state) = self.state_mut() {
            state.metadata = None;
        }
        cx.spawn(async move |entity, cx| {
    ...
    fn matrix_view(&self, window: &Window) -> gpui::AnyElement {
    ...
    fn ensure_anim_loop(&mut self, cx: &mut Context<Self>) {
        let Some(state) = self.state_mut() else {
            return;
        };
...
fn load_metadata(path: &Path) -> AudioMetadata {
    ...
    let cover = build_cover_image(tag);
...
fn build_handle(initial_path: PathBuf) -> AudioHandle {
...
fn build_cover_image(tag: &lofty::tag::Tag) -> Option<Arc<Image>> {
    tag.pictures()
        .iter()
